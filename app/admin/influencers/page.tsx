...
import {
  LogOut, Search, Check, X, Eye, RefreshCw, Wallet,
...
export default function AdminInfluencersPage() {
  ...
  const [pFilter, setPFilter] = useState<'all'|'initiated'|'processing'|'paid'|'failed'|'canceled'>('initiated');
  const [pLoading, setPLoading] = useState(true);
  const [pRefreshKey, setPRefreshKey] = useState(0);
  const [payouts, setPayouts] = useState<PayoutRow[]>([]);
  const [influencerMap, setInfluencerMap] = useState<Record<string, { name: string; handle?: string|null }>>({});
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (error) { toast.error(error.message); router.replace('/admin'); return; }
  ...
  // ====== LOAD: Influencer requests (existing) ======
  useEffect(() => {
    ...
    (async () => {
      ...
      if (irErr) { setLoading(false); toast.error(irErr.message); return; }
      ...
      if (ids.length) {
        ...
        if (profErr) { setLoading(false); toast.error(profErr.message); return; }
  ...
  const setStatus = async (row: IR, status: IRStatus) => {
    if (status === 'approved') {
      const { error } = await supabase.rpc('approve_influencer', { p_request_id: row.id });
    ...
    if (status === 'rejected') {
      const { error } = await supabase.rpc('reject_influencer', { p_request_id: row.id });
      if (error) { toast.error(error.message); return; }
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data, error } = await supabase
        ...
        .select('id, influencer_id, amount, currency, status, notes, created_at, paid_at, covering_orders')
        .order('created_at', { ascending: false });
      if (error) { setPLoading(false); toast.error(error.message); return; }
  ...
  const payoutFiltered = useMemo(() => {
    ...
    return payouts.filter(r => {
      ...
      const inString = `${who?.name || ''} ${who?.handle || ''} ${r.id} ${r.influencer_id} ${r.notes || ''}`.toLowerCase();
  ...
  const payoutBadge = (s: PayoutRow['status']) => {
    switch (s) {
      case 'initiated': return <Badge className="bg-amber-50 text-amber-700 border border-amber-200">Pending</Badge>;
      case 'processing': return <Badge className="bg-sky-50 text-sky-700 border border-sky-200">Processing</Badge>;
      case 'paid': return <Badge className="bg-emerald-50 text-emerald-700 border border-emerald-200">Settled</Badge>;
    ...
    <div className="min-h-screen bg-muted/30">
      <header className="border-b bg-background">
        <div className="container mx-auto py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            <Button variant="outline" size="sm" onClick={() => setTab('requests')}>Requests</Button>
            <Button variant="outline" size="sm" onClick={() => setTab('payouts')}>Payouts</Button>
      ...
      <div className="container mx-auto py-8">
        {/* ===================== TAB: REQUESTS (original) ===================== */}
        {tab === 'requests' && (
          <Card>
            <CardHeader>
              <CardTitle>Influencer requests</CardTitle>
              <CardDescription>Review applications and manage approved creators</CardDescription>
            </CardHeader>
            <CardContent>
              {/* Controls */}
                      ...
                      return (
                        <TableRow key={r.id}>
                          <TableCell className="font-medium">
                            <div>{p?.full_name || '—'}</div>
                            <div className="text-xs text-muted-foreground">{r.user_id.slice(0,8)}…{r.user_id.slice(-4)}</div>
                          </TableCell>
                          <TableCell>
                            <div>{r.handle || '—'}</div>
                          ...
                          <TableCell className="text-right">
                            <div className="flex items-center justify-end gap-2">
                              <Button
                                size="sm"
                                variant="outline"
                                onClick={() => router.push(`/admin/influencers/${r.user_id}`)}
                              >
        ...
        {tab === 'payouts' && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>Approve and settle influencer withdrawals</CardDescription>
            </CardHeader>
            <CardContent>
              {/* Controls */}
              <div className="mb-4 flex flex-col md:flex-row gap-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {(['initiated','processing','paid','failed','canceled','all'] as const).map(s => (
                    <Button key={s} variant={pFilter===s ? 'default' : 'outline'} onClick={()=>setPFilter(s)} className="capitalize">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <div>
                                ...
                                <div className="text-xs text-muted-foreground">
                                  @{who?.handle || '—'} • {r.influencer_id.slice(0,6)}…{r.influencer_id.slice(-4)}
                                </div>
                              </div>
                            </div>
                          </TableCell>
                          <TableCell className="whitespace-nowrap">
                            <div className="font-semibold">{toINR(r.amount, r.currency)}</div>
                            {r.covering_orders && r.covering_orders.length > 0 && (
                              <div className="text-[11px] text-muted-foreground">orders: {r.covering_orders.length}</div>
                            )}
                          </TableCell>
                          <TableCell className="whitespace-nowrap">
                            <div>{new Date(r.created_at).toLocaleString()}</div>
                            {r.status === 'paid' && r.paid_at && (
                              <div className="text-xs text-emerald-700">Paid {new Date(r.paid_at).toLocaleString()}</div>
                            )}
                          </TableCell>
                            ...
                            {noteEditing?.id === r.id ? (
                              <div className="flex items-center gap-2">
                                <Input
                                  ...
                                  onChange={(e)=>setNoteEditing({ id: r.id, note: e.target.value })}
                                  placeholder="Internal note / reference"
                                />
                                <Button size="sm" onClick={()=>savePayoutNote(r.id, noteEditing.note)} disabled={updatingId===r.id}>
                                  {updatingId===r.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <CheckCircle2 className="h-4 w-4" />}
                                </Button>
                                <Button size="sm" variant="outline" onClick={()=>setNoteEditing(null)}><X className="h-4 w-4" /></Button>
                              </div>
                            ) : (
                              <div className="flex items-center gap-2">
                                <div className="truncate text-sm">{r.notes || <span className="text-muted-foreground">—</span>}</div>
                                <Button size="icon" variant="ghost" onClick={()=>setNoteEditing({ id: r.id, note: r.notes || '' })}>
                          ...
                          <TableCell className="text-right">
                            <div className="flex flex-wrap items-center justify-end gap-2">
                              <Button
                                size="sm"
                                variant="outline"
                                disabled={updatingId===r.id || r.status==='processing' || r.status==='paid'}
                                onClick={()=>setPayoutStatus(r, 'processing')}
                              >
                                {updatingId===r.id ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <AlertCircle className="mr-2 h-4 w-4" />} Processing
                              </Button>
                              <Button
                                size="sm"
                                variant="default"
                              ...
                              >
                                {updatingId===r.id ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <CheckCircle2 className="mr-2 h-4 w-4" />} Mark paid
                              </Button>
                              <Button
                                size="sm"
                                variant="destructive"
